...
> 文档状态：已按「5 Tab + 6 能力域 + 全局入口」重构；**§5.1 / §4.10 已与 `images/rent/` 设计稿对齐**
...
- **房源详情（P-02）**展示本月待收 / 待付及收支简卡，作为单房经营摘要入口。
...
| **收租周期** | `网银转账 · 每4周` / `Bank · 4 wk` | 每 N 周收一次（示例 N=4），与合同收款方式同区展示 |
| **租金涵盖** | 绿色条：`租金涵盖 2026年1月1日 – 1月28日` / `1 Jan – 28 Jan` | 本账单覆盖的租期日历日；右侧 **日历按钮** 进入全屏月历 |
| **应付截止** | 金额右侧：`截至日期2026/1/1` / `Due 1 Jan 2026` | **期初当日**须付清（非期末）；中文日期用 `YYYY/M/D`，无「最晚当日付清」副文案 |
| **租住类型** | 标签 `单间出租` / `Room rental` | 示例为分租；**不**再展示「非整租」等辅助标签 |
| **物业地址** | 青色可点地址 + **复制** 图标 | 点地址 → 唤起系统地图导航；点复制 → 复制地址文本 |
| **租住单元** | `次卧 B · 共用厨卫` 等一行 + `›` | 点击进入单元详情（设计稿未展开下级页） |
...
| 区域 | 中文 | 英文 |
|------|------|------|
| 导航栏 | 标题「收租」；右侧 📊、🔔(角标数字) | Title **Rent**；**Stats**、**Alerts** |
| 紧急横幅 | 橙底卡片：「紧急待办」+ 交房/到期摘要 + `›` | **Urgent** + move-in / lease summary |
| 本月概览 | 标题 + 右上**当前月份**（如「五月」/ **May**）；**待收**、**已收**两卡并排，大号金额 | **This Month** + month name；**Due** / **Received** cards |
| 待处理列表 | 标题 + 数量角标；每行：类型图标、标题、**房源 · 租客**、状态标签、金额、`›` | **To Do** + count；row layout same pattern |
| 状态标签色 | 租金待收（青）、押金待退（橙）、附加费待收（蓝）等 | **Due** (teal)、**Refund** (orange)、fee **Due** (blue) |
| 列表底链 | 「全部应收 ›」 | **All Receivables ›** |
| 底部 Tab | 收租（选中）、支出、租客、房源、更多 | Rent (active)、Expenses、Tenants、Properties、More |

**R-03 应收详情**（从 R-01 待处理卡片进入；Pencil 画板名 `r-02-receivable-detail-*`）

| 中文 | 英文 |
|------|------|
| ![R-03 应收详情（中文）](images/rent/r-02-receivable-detail-zh.png) | ![R-03 应收详情（英文）](images/rent/r-02-receivable-detail-en.png) |

> 画板 `r-02-receivable-detail-zh` / `r-02-receivable-detail-en` · 页面编号 **R-03**

#### R-03 应收详情 — 界面要素（以设计稿为准）

| 区块 | 中文 | 英文 |
|------|------|------|
| 导航栏 | `‹` 返回；标题「应收详情」；📊、🔔 | Back；**Receivable**；stats、alerts |
| 金额卡 | 类型「租金」+ 状态「待收」；大号 **`$1,600`**；右上 **`截至日期2026/1/1`**（橙色） | **Rent** + **Due**；**`$1,600`**；**`Due 1 Jan 2026`** |
| 涵盖条 | 绿底条：**租金涵盖 2026年1月1日 – 1月28日** + 右侧日历按钮 | Green bar：**`1 Jan – 28 Jan`** + calendar button |
| 合同信息 | 收款方式 **网银转账 · 每4周**；合同截止 **2027年12月31日**；**查看合同原件** `›` | **Payment** **Bank · 4 wk**；**Lease ends** **31 Dec 2027**；**View lease document** |
| 房源卡 | 标题「房源与租住单元」；标签 **单间出租**；**青色地址**（地图 pin，可点导航）+ **复制**；单元 **次卧 B · 共用厨卫** `›` | **Property & tenancy**；**Room rental**；teal **address** + **copy**；**Room B · Shared kitchen** |
| 租客卡 | 「租客（2 人）」；列表项（头像首字 + 姓名 + `›`） | **Tenants (2)**；rows with avatar + name |
| 备注 | 「备注」+ **编辑**；正文 + 灰色「随时可改」 | **Notes** + **Edit**；note + **Editable anytime** |
| 底栏 | 全宽主按钮 **确认收款**（固定底栏，无多余留白） | **Confirm Payment** |

**租金涵盖日期**（R-03 金额卡日历按钮 → 全屏子页）
...
#### R-03 租金涵盖日期 — 界面要素（以设计稿为准）

| 要素 | 说明 |
|------|------|
| 导航 | **返回** + 标题「租金涵盖日期」/ **Rent coverage** |
| 周标题 | 顶部固定一行：一～日 / M～S |
| 月列表 | **连续滚动**多个月（示例：2025年12月 → 2026年1月 → 2026年2月），无左右切月按钮 |
| 高亮 | 本期涵盖日（示例 1–28 日）浅绿底；**账期首日**橙框（截止日） |
| 非涵盖日 | 当月其余日期默认样式；其他月份无涵盖高亮 |

| 编号 | 页面 | 说明 | 设计稿 |
|------|------|------|--------|
| R-01 | 收租总览 | 见上文 **R-01 界面要素**；默认落地 Tab | [中文](images/rent/r-01-rent-home-zh.png) · [英文](images/rent/r-01-rent-home-en.png) |
| R-02 | 应收列表 | 租金、押金、附加收费、赔偿；筛选：状态 / 房源 / 租客（**尚无设计稿**） | — |
| R-03 | 应收详情 | 见上文 **R-03 界面要素** + §4.10；确认收款 → R-04 | [详情 中文](images/rent/r-02-receivable-detail-zh.png) · [详情 英文](images/rent/r-02-receivable-detail-en.png) · [月历 中文](images/rent/r-03-coverage-calendar-zh.png) · [月历 英文](images/rent/r-03-coverage-calendar-en.png) |
| R-03a | 租金涵盖日期 | 应收详情日历按钮进入的全屏月历；见 **R-03 租金涵盖日期 — 界面要素** | 同上「月历」列 |
| R-04 | 确认收款 Sheet | 租金等应收：一键确认到账，支持部分收 |
...
| P-02 | 房源详情 | 基本信息、当前租约与租客、**本月待收 / 待付**及收支简卡 |
...
1. 系统按租约自动生成本期租金应收。
2. 到期前通过提醒策略通知房东。
3. 房东在 **收租** 待处理列表点进 **应收详情**（R-03），可查看截至日期、租金涵盖区间；点 **日历按钮** 进入 **租金涵盖日期**全屏月历核对涵盖日。
4. 房东收款后在 R-03 底部点击 **确认收款**（进入 R-04 Sheet，设计稿待补）。
...
源文件：`pencil/rentimate-rent.pen`（iPhone 14 Pro，393×852）
...
| R-03 | `r-02-receivable-detail-zh` | [images/rent/r-02-receivable-detail-zh.png](images/rent/r-02-receivable-detail-zh.png) | 应收详情：$ 金额、截至/涵盖、单间出租、地址导航+复制 · 中文 |
| R-03 | `r-02-receivable-detail-en` | [images/rent/r-02-receivable-detail-en.png](images/rent/r-02-receivable-detail-en.png) | 应收详情 · 英文 |
| R-03a | `r-03-coverage-calendar-zh` | [images/rent/r-03-coverage-calendar-zh.png](images/rent/r-03-coverage-calendar-zh.png) | 租金涵盖日期：连续滚动月历、涵盖高亮 · 中文 |
| R-03a | `r-03-coverage-calendar-en` | [images/rent/r-03-coverage-calendar-en.png](images/rent/r-03-coverage-calendar-en.png) | 租金涵盖日期 · 英文 |

在 Pencil 中打开 `pencil/rentimate-rent.pen` 可编辑画板；在 README / GitHub 中通过 `images/` 下 PNG 预览。
...
**最后更新**：2026-05-29（收租 R-01 / R-03 / R-03a 设计稿同步）
